...
small_gicp::RegistrationSetting makeGicpSettings() {
  ...
  settings.downsampling_resolution = 0.5;
  settings.max_correspondence_distance = 2.0;
  return settings;
}
...
std::vector<LidarRelativePose> computeLidarOdometry(
  ...
  const auto gicp_settings = makeGicpSettings();

  auto prev_points = toEigenPoints(lidar_scans[0].data);
  auto [prev_cloud, prev_tree] = small_gicp::preprocess_points(
      prev_points, gicp_settings.downsampling_resolution, 10, gicp_settings.num_threads);
  ...
  for (std::size_t i = 1; i < lidar_scans.size(); ++i) {
    ...
    auto [curr_cloud, curr_tree] = small_gicp::preprocess_points(
        curr_points, gicp_settings.downsampling_resolution, 10, gicp_settings.num_threads);

    auto registration = small_gicp::align(*prev_cloud, *curr_cloud, *prev_tree,
                                          Eigen::Isometry3d::Identity(), gicp_settings);

    core::LidarDelta T_lidar(isometryToPose3(registration.T_target_source));
    ...
    results.push_back({
        ...
        .num_inliers = static_cast<int>(registration.num_inliers),
    });
