...
static NoiseModels makeNoiseModels() {
  NoiseModels models;
  models.pose =
      noiseModel::Diagonal::Sigmas((Vector(6) << 0.05, 0.05, 0.05, 0.5, 0.5, 0.5).finished());
  models.velocity = noiseModel::Isotropic::Sigma(3, 0.1);
  models.bias =
  ...
  models.gps = noiseModel::Robust::Create(huber, base);
  models.lidar =
      noiseModel::Diagonal::Sigmas((Vector(6) << 0.05, 0.05, 0.05, 0.1, 0.1, 0.1).finished());
  return models;
}
...
static void insertInitialValues(Values& values, const std::vector<KeyframeEstimate>& estimates) {
