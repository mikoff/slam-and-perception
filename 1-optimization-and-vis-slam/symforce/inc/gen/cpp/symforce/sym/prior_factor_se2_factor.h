 ...
 *     jacobian: (3x3) jacobian of res wrt arg T_a (3)
 *     hessian: (3x3) Gauss-Newton hessian for arg T_a (3)
 *     rhs: (3x1) Gauss-Newton rhs for arg T_a (3)
 */
template <typename Scalar>
void PriorFactorSe2Factor(const sym::Pose2<Scalar>& T_a, const sym::Pose2<Scalar>& T_a_prior,
  ...
  const Eigen::Matrix<Scalar, 4, 1>& _T_a_prior = T_a_prior.Data();

  // Intermediate terms (19)
  const Scalar _tmp0 = _T_a[2] - _T_a_prior[2];
  const Scalar _tmp1 = _T_a[3] - _T_a_prior[3];
  const Scalar _tmp2 = _T_a[0] * _T_a_prior[1];
  const Scalar _tmp3 = _T_a[1] * _T_a_prior[0];
  const Scalar _tmp4 = -_tmp2 + _tmp3;
  const Scalar _tmp5 = _T_a[0] * _T_a_prior[0] + _T_a[1] * _T_a_prior[1];
  ...
  const Scalar _tmp13 = _tmp5 / _tmp6 - _tmp4 * (_tmp2 - _tmp3) / _tmp11;
  ...
  const Scalar _tmp18 =
      std::pow(_tmp13, Scalar(2)) * std::pow(_tmp6, Scalar(4)) / std::pow(_tmp12, Scalar(2));
