...
sym::Factor<double> SE2::buildBetweenFactor(
  ...
  return sym::Factor<double>::Hessian(
      ...
      {{'P', m.id_from}, {'P', m.id_to}, {'m', k}, {'I', k}, {'e'}},
      {{'P', m.id_from}, {'P', m.id_to}});
}
...
sym::Factor<double> SE2::buildLandmarkFactor(
  ...
  return sym::Factor<double>::Hessian(
      ...
      {{'P', m.pose_id}, {'o', k}, {'L', m.landmark_id}, {'J', k},
       {'b', k}, {'B', k}, {'e'}},
      {{'P', m.pose_id}, {'L', m.landmark_id}});
}

nlohmann::json SE2::poseToJson(int id, const Pose& pose) {
...
nlohmann::json SE2::landmarkToJson(int id, const Position& pos) {
  return {{"id", id}, {"x", pos[0]}, {"y", pos[1]}};
}
...
nlohmann::json SE3::poseToJson(int id, const Pose& pose) {
  const auto& d = pose.Data();
  return {{"id", id},
          {"x", d[4]}, {"y", d[5]}, {"z", d[6]},
...
nlohmann::json SE3::landmarkToJson(int id, const Position& pos) {
  return {{"id", id}, {"x", pos[0]}, {"y", pos[1]}, {"z", pos[2]}};
}
